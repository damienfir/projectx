...
function intent(DOM, HTTP) {
  return {
    createdAlbum$: (0, _helpers.jsonPOST)(HTTP, /\/collections\/\d+\/pages\?index=.*/).map(function (x) {
      return (0, _immutable.fromJS)(x);
    }),
    downloadedAlbum$: (0, _helpers.jsonPOSTResponse)(HTTP, /\/collections\/\d+\/download/).map(function (x) {
      return (0, _immutable.fromJS)(x);
    }),
    shuffledPage$: (0, _helpers.jsonPOST)(HTTP, /\/collections\/\d+\/page\/\d+\?index=.*/).map(function (x) {
      return (0, _immutable.fromJS)(x);
    }),
    savedPage$: (0, _helpers.jsonPOST)(HTTP, /\/save/),
    shuffle$: _helpers2['default'].btn(DOM, '.shuffle-btn').map(function (ev) {
      return ev.target['data-page'];
    }),
    incrPhotos$: _helpers2['default'].btn(DOM, '.incr-btn').map(function (ev) {
      return ev.target['data-page'];
    }),
    decrPhotos$: _helpers2['default'].btn(DOM, '.decr-btn').map(function (ev) {
      return ev.target['data-page'];
    }),
    removeLastPage$: new _rx2['default'].Subject()
  };
}

var params = {};

var index = function index(page) {
  return page.get('index');
};

function model(DOMactions, actions, collection, editing, upload) {

  actions.shuffledPage$.merge(actions.removeLastPage$).subscribe(function (ev) {
    return editing.actions.cancelExt$.onNext(false);
  });

  var demoAlbum$ = collection.actions.storedAlbum$.map(function (demo) {
    return function (album) {
      return (0, _immutable.fromJS)(demo.pages).sortBy(index);
    };
  });

  var albumUpdated$ = actions.createdAlbum$.map(function (newpage) {
    return function (album) {
      return album.push(newpage).sortBy(index);
    };
  });

  var removeLastPage$ = actions.removeLastPage$.map(function (p) {
    return function (album) {
      return album.pop();
    };
  });

  var albumPageShuffled$ = actions.shuffledPage$.map(function (page) {
    return function (album) {
      return album.update(page.get('index'), function (p) {
        return page.update('tiles', function (tiles) {
          return tiles.map(function (tile) {
            var samePhotoID = function samePhotoID(t) {
              return t.get('photoID') === tile.get('photoID');
            };
            var rot = (p.get('tiles').find(samePhotoID) || album.reduce(function (tiles, page) {
              return tiles.concat(page.get('tiles'));
            }, (0, _immutable.List)()).find(samePhotoID) || (0, _immutable.Map)({ rot: 0 })).get('rot');
            return (0, _immutable.fromJS)(utils.rotateTile(tile.toJS(), rot));
          });
        });
      });
    };
  ...
  // Editing
  var swapTiles$ = editing.actions.swap$.map(function (swap) {
    return function (album) {
      return (0, _immutable.fromJS)(utils.swapTiles(album.toJS(), [swap.from.page, swap.from.idx], [swap.to.page, swap.to.idx]));
    };
  });

  var dragPhoto$ = editing.actions.drag$.map(function (drag) {
    return function (album) {
      var moved_x = utils.move(album.getIn([drag.page, 'tiles', drag.idx]).toJS(), drag.dx, 'cx1', 'cx2');
      return album.setIn([drag.page, 'tiles', drag.idx], (0, _immutable.fromJS)(utils.move(moved_x, drag.dy, 'cy1', 'cy2')));
    };
  });

  var rotatePhoto$ = editing.actions.rotate$.withLatestFrom(editing.state$, function (ev, editing) {
    return function (album) {
      return album.updateIn([editing.selected.page, 'tiles', editing.selected.idx], function (tile) {
        return (0, _immutable.fromJS)(utils.rotateTile(tile.toJS()));
      });
    };
  });

  var clickEdge$ = editing.actions.clickEdge$.map(function (ev) {
    return function (album) {
  ...
  var dragEdge$ = editing.actions.dragEdge$.map(function (drag) {
    return function (album) {
      return album.updateIn([params.page, 'tiles'], function (tiles) {
        return utils.dragTiles(tiles, params, drag.dx, drag.dy);
      });
    };
  });
  // dragEdge$.subscribe(x => console.log(x));

  var state$ = Observable.merge(albumUpdated$, demoAlbum$, albumPageShuffled$, swapTiles$, dragPhoto$, removeLastPage$, rotatePhoto$, clickEdge$, dragEdge$).startWith((0, _immutable.List)()).scan(_helpers.apply).shareReplay(1);
...
function photosFromTiles(photos, tiles) {
  return _.filter(photos, function (p) {
    return _.where(tiles, { 'photoID': p.id }).length > 0;
  });
}

function chooseNPics(page) {
  if (page === 0) return 1;else if (page === 1) return 3;else return Math.floor(Math.random() * 4 + 1);
}

function requests(DOMactions, actions, album$, photos, upload, editing) {

  var cover$ = editing.actions.cover$.withLatestFrom(editing.state$, function (ev, _ref) {
  ...
  var moveTile$ = editing.actions.move$.merge(cover$).withLatestFrom(album$, photos.state$, function (move, album, photos) {
    var from = { page: album.get(move.from.page).toJS() };
    var to = { page: move.to.page >= 0 ? album.get(move.to.page).toJS() : undefined };
    var tile = from.page.tiles[move.from.idx];
    if (_.isUndefined(tile)) return false;
    var photoID = tile.photoID;

    var photosFrom = photosFromTiles(photos, from.page.tiles);
    if (_.isUndefined(to.page)) {
      to.photos = [];
      from.photos = _.uniq(photosFrom.filter(function (p) {
        return p.id !== photoID;
      }));
    } else {
    ...
    to.photos = to.photos.concat(photos.filter(function (p) {
      return p.id === photoID;
    }));
  ...
  }).filter(_.identity).flatMap(function (reqs) {
    return _rx2['default'].Observable.fromArray(reqs);
  }).share();

  var removeTile$ = editing.actions.remove$.withLatestFrom(album$, photos.state$, editing.state$, function (ev, album, photos, _ref2) {
    var selected = _ref2.selected;

    if (_.isUndefined(selected) || _.isUndefined(album.get(selected.page))) return false;
    var page = album.get(selected.page).toJS();
    return {
      page: page,
      photos: photosFromTiles(photos, page.tiles).filter(function (p) {
        return p.id !== page.tiles[selected.idx].photoID;
      })
    };
  }).filter(_.identity);

  var shufflePage$ = actions.shuffle$.withLatestFrom(photos.state$, album$, function (index, photos, album) {
    var page = album.get(index, (0, _immutable.Map)()).toJS();
    console.log(page);
    if (!page) return {};
    return {
      page: page,
  ...
  var _moveTile$$partition = moveTile$.partition(function (req) {
    return !_.isUndefined(req.page);
  });

  var _moveTile$$partition2 = _slicedToArray(_moveTile$$partition, 2);

  var pageExist$ = _moveTile$$partition2[0];
  var pageNew$ = _moveTile$$partition2[1];

  var generatePage$ = pageExist$.merge(removeTile$).merge(shufflePage$);

  var addPage$ = pageNew$.map(function (_ref3) {
    var page = _ref3.page;
    var photos = _ref3.photos;
    return { photos: photos, page: -1 };
  });

  var albumSize$ = album$.map(function (album) {
    return album.size;
  });

  var photosGroups$ = upload.actions.startUpload$.withLatestFrom(albumSize$, _helpers2['default'].argArray).flatMapLatest(function (_ref4) {
    var _ref42 = _slicedToArray(_ref4, 2);

    var _ref42$0 = _slicedToArray(_ref42[0], 2);

    var ev = _ref42$0[0];
    var collection = _ref42$0[1];
    var page = _ref42[1];

    return _rx2['default'].Observable.from(ev).scan(function (acc, el) {
      return acc.concat(el);
    }, []).scan(function (prev, files) {
      var newPhoto = page === 1 ? files.slice(-2) : files.slice(-1);
      var remainingPhotos = ev.length - files.length + 1;
      ...
      var npics = _ref5.npics;
      var photos = _ref5.photos;
      return photos.length === npics;
    }).concatMap(function (obj) {
      return _rx2['default'].Observable.fromArray(obj.photos).flatMap(function (photo) {
        return makeUploadRequest(photo, collection).tap(function (p) {
          return upload.actions.uploaded$.onNext(p);
        }).tap(function (p) {
          return photos.actions.uploadedPhoto$.onNext(p);
        });
      }).reduce(function (photos, photo) {
        return photos.concat(photo);
      }, []).map(function (photos) {
        return { page: obj.page, photos: photos };
      });
    });
  }).share();

  // remove last page if empty
  generatePage$.withLatestFrom(albumSize$, function (req, size) {
    return req.page.index === size - 1 && req.photos.length === 0;
  }).filter(_.identity).subscribe(function (r) {
    return actions.removeLastPage$.onNext();
  });

  return {
    createAlbum$: photosGroups$.merge(addPage$),
    generatePage$: generatePage$.filter(function (req) {
      return req.photos.length > 0 && !_.isUndefined(req.page);
    })
  };
}
...
function makeRequests(req, album$, collection) {
  ...
  return {
    createAlbum$: req.createAlbum$.withLatestFrom(collection.state$, albumSize$, function (group, collection, size) {
    ...
    generatePage$: req.generatePage$.withLatestFrom(collection.state$, function (req, collection) {
      return {
...
function ui(actions, requests) {
  ...
  var shuffling$ = requests.generatePage$.map(function (req) {
    return req.page.index;
  }).map(function (page) {
    return function (state) {
      return state.update('shuffling', function (a) {
        return a.push(page);
      });
    };
  }).merge(actions.shuffledPage$.map(function (page) {
    return page.get('index');
  }).map(function (page) {
    return function (state) {
      return state.update('shuffling', function (a) {
  ...
  var model$ = _rx2['default'].Observable.merge(shuffling$);

  return model$.startWith(initialState).scan(_helpers.apply).shareReplay(1);
}

module.exports = function (DOM, HTTP, DOMactions, collection, photos, upload, editing) {
  var actions = intent(DOM, HTTP);
  var state$ = model(DOMactions, actions, collection, editing, upload);
  var req = requests(DOMactions, actions, state$, photos, upload, editing);
  var ui$ = ui(actions, req);
  var http = makeRequests(req, state$, collection);
  var vtree$ = (0, _view2['default'])(state$, collection.state$, editing.state$, ui$);
...
},{"../helpers":9,"./utils":3,"./view":4,"@cycle/dom":22,"immutable":93,"rx":101}],2:[function(require,module,exports){
'use strict';
...
var eventToCoord = function eventToCoord(ev) {
  return {
    'x': ev.screenX,
    'y': ev.screenY,
    'img': ev.target,
...
function intent(DOM) {
  var tileDown$ = DOM.select('.ui-tile, .page-hover').events('mousedown').map(_helpers2['default'].cancel).map(eventToCoord);
  var tileUp$ = DOM.select('.ui-tile, .page-hover').events('mouseup').map(eventToCoord);
  var mouseUp$ = DOM.select(':root').events('mouseup');
  var mouseMove$ = DOM.select(':root').events('mousemove').map(_helpers2['default'].cancel);
  var mouseMoveCoord$ = mouseMove$.map(eventToCoord);
  // let edgeDown$ = DOM.select('.move-mosaic').events('mousedown')
  //   .filter(ev => _.contains(ev.target.classList, 'move-mosaic'));
  var edgeDown$ = DOM.select('.node').events('mousedown').map(_helpers2['default'].cancel);

  var cancelExt$ = new _rx2['default'].Subject();

  var remove$ = _helpers2['default'].btn(DOM, '#remove-btn');
  var cancelBtn$ = _helpers2['default'].btn(DOM, '#cancel-btn');
  var rotate$ = _helpers2['default'].btn(DOM, '#rotate-btn');
  var cover$ = _helpers2['default'].btn(DOM, '#add-cover-btn');

  var clickEdge$ = edgeDown$.map(function (ev) {
    return {
  ...
  var dragEdge$ = clickEdge$.flatMapLatest(function (down) {
  ...
  var cancel$ = tileDown$.flatMapLatest(function (down) {
    return mouseMove$.takeUntil(mouseUp$).take(1);
  }).merge(cancelBtn$).map(false).share();

  var selected$ = tileDown$.flatMapLatest(function (down) {
    return mouseMove$.takeUntil(mouseUp$).count().filter(function (c) {
      return c === 0;
    }).map(down);
  }).merge(cancel$).scan(function (prev, x) {
    return prev ? false : x;
  }, false).filter(_.identity).share();

  var _selected$$flatMapLatest$partition = selected$.flatMapLatest(function (from) {
    return tileUp$.take(1).takeUntil(cancel$).map(function (to) {
      return { from: from, to: to };
    });
  }).partition(function (_ref4) {
    var from = _ref4.from;
    var to = _ref4.to;
    return from.page === to.page;
  });

  var _selected$$flatMapLatest$partition2 = _slicedToArray(_selected$$flatMapLatest$partition, 2);

  var swap$ = _selected$$flatMapLatest$partition2[0];
  var move$ = _selected$$flatMapLatest$partition2[1];

  var drag$ = tileDown$.flatMapLatest(function (down) {
    return mouseMoveCoord$.takeUntil(mouseUp$).pairwise().map(function (_ref3) {
  ...
  return { swap$: swap$, move$: move$, drag$: drag$, selected$: selected$, cancel$: cancel$, clickEdge$: clickEdge$, dragEdge$: dragEdge$, remove$: remove$, rotate$: rotate$, cover$: cover$, cancelExt$: cancelExt$ };
}

function model(actions, DOMactions) {

  actions.clickEdge$.subscribe(function (ev) {
    return $('.node').tooltip('destroy');
  });
  actions.selected$.subscribe(function (ev) {
    return $('.ui-tile').tooltip('destroy');
  });

  return _rx2['default'].Observable.merge(actions.selected$.map(function (down) {
    return function (state) {
      return _.extend(state, { selected: down });
    };
  }), actions.selected$.take(1).map(function (ev) {
    return function (state) {
      return _.extend(state, { selectedTile: true });
    };
  }), actions.clickEdge$.take(1).map(function (ev) {
    return function (state) {
      return _.extend(state, { draggedNode: true });
    };
  }), _rx2['default'].Observable.merge(actions.swap$, actions.move$, actions.cancel$, actions.cancelExt$).map(function (x) {
    return function (state) {
      return _.extend(state, { selected: undefined });
    };
  }), DOMactions.reset$.map(function (ev) {
    return function (state) {
      return {};
    };
  })).startWith({}).scan(_helpers2['default'].apply);
}
...
module.exports = function (DOM, DOMactions) {
  ...
  var state$ = model(actions, DOMactions);
...
},{"../helpers":9,"@cycle/dom":22,"rx":101}],3:[function(require,module,exports){
'use strict';
...
exports.rotateTile = rotateTile;
exports.swapTiles = swapTiles;
exports.move = move;
exports.getParams = getParams;
exports.dragTiles = dragTiles;
...
function swapTiles(album, _ref, _ref3) {
  var _ref2 = _slicedToArray(_ref, 2);
...
function getParams(tiles, x, y) {
  var tiles2 = tiles.map(function (t, i) {
    return { t: t, i: i };
  });
  var move_x_tl = tiles2.filter(function (_ref4) {
    var t = _ref4.t;
    var i = _ref4.i;
    return Math.abs(t.get('tx1') - x) < margin;
  }).map(function (_ref5) {
    var t = _ref5.t;
    var i = _ref5.i;
    return i;
  });
  var move_x_br = tiles2.filter(function (_ref6) {
    var t = _ref6.t;
    var i = _ref6.i;
    return Math.abs(t.get('tx2') - x) < margin;
  }).map(function (_ref7) {
    var t = _ref7.t;
    var i = _ref7.i;
    return i;
  });
  var move_y_tl = tiles2.filter(function (_ref8) {
    var t = _ref8.t;
    var i = _ref8.i;
    return Math.abs(t.get('ty1') - y) < margin;
  }).map(function (_ref9) {
    var t = _ref9.t;
    var i = _ref9.i;
    return i;
  });
  var move_y_br = tiles2.filter(function (_ref10) {
    var t = _ref10.t;
    var i = _ref10.i;
    return Math.abs(t.get('ty2') - y) < margin;
  }).map(function (_ref11) {
    var t = _ref11.t;
    var i = _ref11.i;
    return i;
  });
  return { move_x_tl: move_x_tl, move_y_tl: move_y_tl, move_x_br: move_x_br, move_y_br: move_y_br };
}

var d = 0.1;

function dragTiles(tiles, params, dx, dy) {
  var canMoveX = params.move_x_tl.size > 0 && params.move_x_br.size > 0;
  var canMoveY = params.move_y_tl.size > 0 && params.move_y_br.size > 0;
  var newTiles = tiles.map(function (t, i) {
    return params.move_x_tl.includes(i) && canMoveX ? t.update('tx1', function (x) {
      return x + dx;
    }) : t;
  }).map(function (t, i) {
    return params.move_x_br.includes(i) && canMoveX ? t.update('tx2', function (x) {
      return x + dx;
    }) : t;
  }).map(function (t, i) {
    return params.move_y_tl.includes(i) && canMoveY ? t.update('ty1', function (x) {
      return x + dy;
    }) : t;
  }).map(function (t, i) {
    return params.move_y_br.includes(i) && canMoveY ? t.update('ty2', function (x) {
      return x + dy;
    }) : t;
  });

  if (newTiles.map(function (t) {
    return [t.get('tx2') - t.get('tx1'), t.get('ty2') - t.get('ty1')];
  }).some(function (_ref12) {
  ...
  return tiles.map(function (x) {
    ...
    var a = _ref132[0];
    var b = _ref132[1];
    return (0, _immutable.fromJS)(resizeTile(a, b));
  });
}

},{"immutable":93}],4:[function(require,module,exports){
'use strict';
...
var coverpage = (0, _immutable.fromJS)({
  tiles: [],
  index: 0
});

var blankpage = (0, _immutable.fromJS)({
  tiles: [],
  index: -2
});

var leftOrRight = function leftOrRight(index) {
  return index % 2 ? '.pull-right' : '.pull-left';
};
var moveOrNot = function moveOrNot(tiles) {
  return tiles.length === 0 ? '.nomove' : '.move-mosaic';
};

var splitIntoSpreads = function splitIntoSpreads(spreads, page) {
  if (spreads.size > 1 && spreads.last().size < 2) {
    return spreads.update(spreads.size - 1, function (x) {
      return x.push(page);
    });
  } else {
    return spreads.push((0, _immutable.List)([page]));
  }
};
...
var renderTile = function renderTile(tile, tileindex, page, editing) {
  var scaleX = 1 / (tile.cx2 - tile.cx1);
  var scaleY = 1 / (tile.cy2 - tile.cy1);

  var selectedClass = editing.selected ? editing.selected.page === page && editing.selected.idx === tileindex ? '.tile-selected' : '.tile-unselected' : '';

  return (0, _cycleDom.h)('.ui-tile' + selectedClass,
  // _.extend(
  {
    'style': {
...
var renderTitle = function renderTitle(collection, page) {
  return !_.isUndefined(collection.id) && page.index === 0 ? (0, _cycleDom.h)('input.cover-title#album-title', {
    'type': 'text',
    'placeholder': (0, _i18n2['default'])('ui.title'),
    'maxLength': 50,
...
var renderBackside = function renderBackside() {
  return (0, _cycleDom.h)('.backside', "bigpiq");
};

var renderHover = function renderHover(editing, page) {
  return editing.selected && editing.selected.page !== page.index ? (0, _cycleDom.h)('.page-hover', { 'data-page': page.index, 'data-idx': 0 }, (0, _cycleDom.h)('h2.center', page.index === 0 ? (0, _i18n2['default'])('ui.copy') : (0, _i18n2['default'])('ui.move'))) : '';
};
...
var drawNode = function drawNode(shift, dragged) {
  return function (node, key) {
    return (0, _cycleDom.h)('button.node.shadow', _.extend({
      // key,
      style: {
...
var renderNodes = function renderNodes(page, editing) {
  var topleft = page.tiles.filter(function (t) {
    return t.tx1 > 0.01 || t.ty1 > 0.01;
  }).map(function (t) {
    return Node(t.tx1, t.ty1);
  });
  var bottomright = page.tiles
  // .filter(a => topleft
  //     .filter(b => a.tx1 === b.x && a.ty1 === b.y).length === 0)
  .filter(function (t) {
    return t.tx2 < 0.99 || t.ty2 < 0.99;
  }).map(function (t) {
    return Node(t.tx2, t.ty2);
  }).filter(function (a) {
    return topleft.filter(function (b) {
      return Math.abs(a.x - b.x) + Math.abs(a.y - b.y) < 0.1;
    }).length === 0;
  });
  return bottomright.map(drawNode(shift, editing.draggedNode)).concat(topleft.map(drawNode(-shift, editing.draggedNode)));
};

var renderAddPhotos = function renderAddPhotos(page, collection) {
  if (!page.tiles.length) {
    if (page.index === 0 && _.isUndefined(collection.id)) {
      return [
      // h('.add-background'),
      (0, _cycleDom.h)('button.btn.btn-info.btn-lg.btn-step.center.shadow#create-btn', [(0, _cycleDom.h)('i.fa.fa-camera.fa-3x'), (0, _i18n2['default'])('front.upload')])];
    } else if (page.index == -2) {
      return (0, _cycleDom.h)('button.btn.btn-primary.center#addmore-btn', [(0, _cycleDom.h)('i.fa.fa-cloud-upload'), ' ' + (0, _i18n2['default'])('ui.add')]);
    }
  }
};

function renderShuffling() {
  return [(0, _cycleDom.h)('i.fa.fa-refresh.fa-spin.fa-4x.center'), (0, _cycleDom.h)('.shuffling.full')];
}

var renderPage = function renderPage(collection, j, editing, ui) {
  return function (page, i) {
    return (0, _cycleDom.h)('.box-mosaic' + leftOrRight(j * 2 + i), {
      'data-page': page.index
    }, [j === 1 && i === 0 ? renderBackside() : page.tiles.map(function (tile, index) {
      return renderTile(tile, index, page.index, editing);
    }).concat(renderTitle(collection, page)).concat(renderNodes(page, editing)).concat(renderAddPhotos(page, collection)).concat(renderHover(editing, page)).concat(renderToolbar(editing, page)).concat(ui.get('shuffling').includes(page.index) ? renderShuffling() : '')]);
  };
};

var renderToolbar = function renderToolbar(editing, page) {
  return editing.selected && editing.selected.page === page.index ? (0, _cycleDom.h)('.btn-group.toolbar', [page.tiles.length <= 1 ? '' : (0, _cycleDom.h)('button.btn.btn-info.btn-lg#remove-btn', { 'attributes': {
      'data-toggle': 'tooltip',
      'data-placement': 'top',
      'title': (0, _i18n2['default'])('toolbar.remove')
    } }, (0, _cycleDom.h)('i.fa.fa-trash-o')), (0, _cycleDom.h)('button.btn.btn-info.btn-lg#rotate-btn', { 'attributes': {
      'data-toggle': 'tooltip',
      'data-placement': 'top',
      'title': (0, _i18n2['default'])('toolbar.rotate')
    } }, (0, _cycleDom.h)('i.fa.fa-rotate-right')), editing.selected.page !== 0 ? (0, _cycleDom.h)('button.btn.btn-info.btn-lg#add-cover-btn', { 'attributes': {
      'data-toggle': 'tooltip',
      'data-placement': 'top',
      'title': (0, _i18n2['default'])('toolbar.cover')
    } }, [(0, _cycleDom.h)('i.fa.fa-book')]) : '', (0, _cycleDom.h)('button.btn.btn-info.btn-lg#cancel-btn', { 'attributes': {
      'data-toggle': 'tooltip',
      'data-placement': 'top',
      'title': (0, _i18n2['default'])('toolbar.cancel')
    } }, [(0, _cycleDom.h)('i.fa.fa-times')])]) : '';
};

var renderBtn = function renderBtn(ui, j) {
  return function (page, i) {
    var p = j * 2 + i;
    var shuffling = ui.get('shuffling').includes(page.index);
    return (0, _cycleDom.h)(leftOrRight(p), [(0, _cycleDom.h)('span.page', { 'data-page': page.index }, (p === 0 ? (0, _i18n2['default'])('ui.cover') : (0, _i18n2['default'])('ui.page') + " " + (p - 1)) + ' '), page.tiles.length > 1 ? (0, _cycleDom.h)('.btn-group', [(0, _cycleDom.h)('button.btn.btn-primary.shuffle-btn', {
      'data-page': page.index,
      'disabled': shuffling
    }, [(0, _cycleDom.h)('i.fa.fa-refresh'), " " + (0, _i18n2['default'])('ui.shuffle')])]) :
...
var renderSpread = function renderSpread(collection, editing, ui) {
  return function (spread, i) {
    var isCover = spread.length === 1 && spread[0].index === 0;

    return (0, _cycleDom.h)('.row.spread' + (isCover ? '.spread-cover' : ''), [(0, _cycleDom.h)('a.spread-anchor', {
      name: 'spread' + i
    }), isCover ? '' : (0, _cycleDom.h)('.col-xs-1.spread-arrow', (0, _cycleDom.h)('a.btn.btn-link', {
      href: '#spread' + (i - 1)
    }, (0, _cycleDom.h)('i.fa.fa-chevron-left.fa-3x'))), (0, _cycleDom.h)('.spread-paper.shadow.clearfix' + (isCover ? '.col-xs-6.col-xs-offset-3' : '.col-xs-10'), spread.map(renderPage(collection, i, editing, ui))), (0, _cycleDom.h)('.col-xs-1.spread-arrow', (0, _cycleDom.h)('a.btn.btn-link', { href: '#spread' + (i + 1) }, (0, _cycleDom.h)('i.fa.fa-chevron-right.fa-3x'))), (0, _cycleDom.h)('.spread-btn.clearfix' + (isCover ? '.col-xs-6.col-xs-offset-3' : '.col-xs-10.col-xs-offset-1'), spread.map(renderBtn(ui, i)))]);
  };
};
...
function toSpreads(album) {
  var pages = album.size === 0 ? album.push(coverpage) : _immutable.List.of(album.first(), blankpage).concat(album.rest()).push(blankpage);

  return pages.reduce(splitIntoSpreads, (0, _immutable.List)()).toJS();
}

module.exports = function (album$, collection$, editing$, ui$) {
  return album$.map(toSpreads).combineLatest(collection$, editing$, ui$, function (album, collection, editing, ui) {
    // return !_.isUndefined(collection.id) ?
    return (0, _cycleDom.h)('div.container-fluid.album', renderAlbum(album, collection, editing, ui));
    // undefined;
  });
};

},{"../helpers":9,"../i18n":10,"@cycle/dom":22,"immutable":93}],5:[function(require,module,exports){
'use strict';
...
var gaEvent = function gaEvent(category, action, label, value) {
  return ga('send', 'event', category, action, label, value);
};

module.exports = function (DOMactions, user, collection, upload, album, order) {
  var events = [
  // global actions
  ...
  DOMactions.hasHash$.map(function (hash) {
    return { c: 'album', a: 'load', l: hash };
  }),

  // file upload
  DOMactions.filedialog$.withLatestFrom(collection.state$, function (files, collection) {
    return { c: 'files', a: 'upload', l: 'dialog', v: files.length };
  }), DOMactions.drop$.withLatestFrom(collection.state$, function (files, collection) {
    return { c: 'files', a: 'upload', l: 'drop', v: files.length };
  }),

  // album modifications
  collection.actions.albumTitle$.debounce(2000).map(function (title) {
    return { c: 'title', a: 'change', l: title };
  }),

  // album.state$.filter(a => a.length > 1).map({c: 'album', a: 'alter'}),

  album.actions.shuffle$.debounce(2000).map(function (page) {
    return { c: 'album', a: 'shuffle', l: page };
  })];
  ...
  _rx2['default'].Observable.merge(events).subscribe(function (_ref) {
    var c = _ref.c;
    var a = _ref.a;
    var l = _ref.l;
    var v = _ref.v;
    return gaEvent(c, a, l, v);
  });
};

},{"rx":101}],6:[function(require,module,exports){
'use strict';
...
function intent(DOM, HTTP) {
  return {
    createdCollection$: (0, _helpers.jsonPOST)(HTTP, /\/users\/\d+\/collections/),
    storedAlbum$: (0, _helpers.jsonGET)(HTTP, /\/users\/\d+\/albums\/[\w\d-]+/),
    albumTitle$: DOM.select('#album-title').events("input").map(function (ev) {
      return ev.target.value;
    }),
    clickTitle$: DOM.select(".cover-title").events('click'),
    confirmTitle$: _helpers2['default'].btn(DOM, '#title-btn').map(function (ev) {
      return document.getElementById("album-title-front").value;
    })
  };
}

function events(DOMactions, actions) {
  actions.clickTitle$.subscribe(function (ev) {
    return document.getElementById("album-title").focus();
  });
  DOMactions.reset$.subscribe(function (x) {
    return window.history.pushState({}, '', '/ui');
  });
}

var replaceName = function replaceName(c) {
  return c.name ? c : _.extend(c, { name: null });
};

function model(actions, DOMactions) {
  var storedCollection$ = actions.storedAlbum$.map(replaceName).map(function (demo) {
    return function (col) {
      return demo.collection;
    };
  });
  var collectionUpdated$ = actions.createdCollection$.map(replaceName).map(function (col) {
    return function (collection) {
      return col;
    };
  });
  var clearCollection$ = DOMactions.reset$.map(function (x) {
    return function (item) {
      return initialState;
    };
  });
  var collectionName$ = actions.albumTitle$.merge(actions.confirmTitle$).map(function (name) {
    return function (collection) {
      return _.extend(collection, { name: name });
    };
  });

  actions.createdCollection$.merge(actions.storedAlbum$.map(function (d) {
    return d.collection;
  })).subscribe(function (col) {
    return window.history.pushState({}, '', '/ui/' + col.hash);
  });

  return Observable.merge(collectionUpdated$, clearCollection$, storedCollection$, collectionName$).startWith(initialState).scan(_helpers.apply).shareReplay(1);
}

function requests(DOMactions, user, collection$) {
  return {
    createCollection$: DOMactions.selectFiles$.flatMapLatest(function (files) {
      return user.state$.filter(_helpers.hasID).take(1).map(function (user) {
        return [files, user];
      });
    }).withLatestFrom(collection$, _helpers.argArray).filter(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 2);

      var x = _ref2[0];
      var col = _ref2[1];
      return _.isUndefined(col.id);
    }).map(function (_ref3) {
      var _ref32 = _slicedToArray(_ref3, 2);
    ...
    storedAlbum$: DOMactions.hasHash$.flatMapLatest(function (hash) {
      return user.state$
      // .filter(user => user.id)
      .take(1).map(function (user) {
        return '/users/' + (user.id || 0) + '/albums/' + hash;
      });
    })
  };
}
...
module.exports = function (DOM, HTTP, DOMactions, user) {
  var actions = intent(DOM, HTTP);
  var state$ = model(actions, DOMactions);
  var requests$ = requests(DOMactions, user, state$);
...
},{"./helpers":9,"rx":101}],7:[function(require,module,exports){
"use strict";
...
var cancelDefault = function cancelDefault(ev) {
  ev.preventDefault();
  ev.stopPropagation();
  return ev;
};

var demoID = document.getElementById("demo-id").value;
...
var ascIndex = function ascIndex(a, b) {
  return asc(a.index, b.index);
};

var apply = function apply(state, func) {
  return func(state);
};

var btn = function btn(DOM, selector) {
  return DOM.select(selector).events('click').map(cancelDefault);
};
...
var argArray = function argArray(a, b) {
  return [a, b];
};

var hasNoID = function hasNoID(x) {
  return _.isUndefined(x.id);
};

var hasID = function hasID(x) {
  return !_.isUndefined(x.id) && x.id !== null;
};

var jsonGETResponse = function jsonGETResponse(HTTP, regex) {
  return HTTP.filter(function (res$) {
    return res$.request.method === undefined;
  }).filter(function (res$) {
    return res$.request.match(regex);
  }).mergeAll().share();
};

var jsonGET = function jsonGET(HTTP, regex) {
  return jsonGETResponse(HTTP, regex).map(function (res) {
    return res.body;
  });
};

var jsonPOSTResponse = function jsonPOSTResponse(HTTP, regex) {
  return HTTP.filter(function (res$) {
    return res$.request.method === 'POST';
  }).filter(function (res$) {
    return res$.request.url.match(regex);
  }).mergeAll().share();
};

var jsonPOST = function jsonPOST(HTTP, regex) {
  return jsonPOSTResponse(HTTP, regex).map(function (res) {
    return res.body;
  });
};

var jsonRes = function jsonRes(HTTP, regex) {
  return HTTP.filter(function (res$) {
    return res$.request.method ? res$.request.url.match(regex) : res$.request.match(regex);
  }).mergeAll().share();
};

var json = function json(HTTP, regex) {
  return jsonRes(HTTP, regex).map(function (res) {
    return res.body;
  });
};

var hashMap = function hashMap(photos) {
  if (!photos) return {};
  if (!_.isArray(photos)) return photos;
  return _.object(photos.map(function (p) {
    return [p.id, p.hash];
  }));
};

var toArray = function toArray(filelist) {
  var list = [];
...
module.exports = {
  ...
  jsonGET: jsonGET,
  jsonPOST: jsonPOST,
  jsonGETResponse: jsonGETResponse,
...
function getLang() {
  var def = 'en';
  var lang = document.documentElement.lang || def;
...
var _albumAlbum = require('./album/album');

var _albumAlbum2 = _interopRequireDefault(_albumAlbum);

var _albumEditing = require('./album/editing');

var _albumEditing2 = _interopRequireDefault(_albumEditing);
...
function parseEntry(f, cb) {
  if (f.isFile) {
    ...
    (function () {
      var entries = [];
      var readEntries = function readEntries(reader) {
...
function intent(DOM, HTTP) {
  var cancelDefault = function cancelDefault(ev) {
    ev.preventDefault();ev.stopPropagation();return ev;
  };
  var btn = function btn(selector) {
    return DOM.select(selector).events('click').map(cancelDefault);
  };

  var listDir = _rx2['default'].Observable.fromCallback(parseEntry);
  var getFile = _rx2['default'].Observable.fromCallback(function (entry, cb) {
    return entry.file(cb);
  });

  var drop$ = DOM.select("#upload-area").events('drop').map(_helpers2['default'].cancel).flatMap(function (ev) {
    return _rx2['default'].Observable.from(_helpers2['default'].toArray(ev.dataTransfer.items));
  }).flatMap(function (dir) {
    return listDir(dir.webkitGetAsEntry ? dir.webkitGetAsEntry() : dir);
  }).map(function (x) {
    return _.flatten(x.filter(function (f) {
      return f !== null;
    }));
  }).flatMap(function (entries) {
    return _rx2['default'].Observable.from(entries).flatMap(function (entry) {
      return getFile(entry);
    }).reduce(function (arr, x) {
      return arr.concat(x);
    }, []);
  });
  var filedialog$ = DOM.select('#file-input').events('change').map(function (ev) {
    return _helpers2['default'].toArray(ev.target.files);
  });
  var selectFiles$ = filedialog$.merge(drop$);
  ...
  var actions = {
    ...
    hasHash$: Observable.just(window.location.pathname.split('/')).filter(function (url) {
      return url.length > 2;
    }).map(function (url) {
      return url.pop();
    }).shareReplay(1)
...
function view(collection, album, upload, order, save, editing) {
  var toolbarDOM = Observable.combineLatest(collection.state$, album.state$, upload.state$, Elements.renderToolbar);
  var buttonDOM = collection.state$.map(Elements.renderStartPage);

  // $('body').tooltip({selector: '[data-toggle="tooltip"]'});

  $(window).on('scroll', function () {
    return !$(document).scrollTop() ? $('#nav').removeClass('shadow-lg') : $('#nav').addClass('shadow-lg');
  });
  ...
  return Observable.combineLatest(toolbarDOM, upload.DOM, album.DOM, order.DOM, buttonDOM, save.DOM, function (toolbarVTree, uploadVTree, albumVTree, orderVTree, buttonVTree, saveVTree) {
    return (0, _cycleDom.h)('div.theme-blue', [toolbarVTree, uploadVTree, orderVTree, albumVTree, saveVTree]);
  });
}
...
function main(_ref) {
  ...
  var DOMactions = intent(DOM, HTTP);

  var user = (0, _user2['default'])(HTTP, DOMactions);
  var collection = (0, _collection2['default'])(DOM, HTTP, DOMactions, user);
  var upload = (0, _upload2['default'])(DOM, DOMactions, collection);
  var photos = (0, _photos2['default'])(DOMactions, upload, collection);
  var editing = (0, _albumEditing2['default'])(DOM, DOMactions);
  var album = (0, _albumAlbum2['default'])(DOM, HTTP, DOMactions, collection, photos, upload, editing);
  var order = (0, _order2['default'])(DOM, HTTP, user, collection);
  var save = (0, _save2['default'])(DOM, HTTP, user, collection, album);

  var requests$ = Observable.merge(_.values(user.HTTP).concat(_.values(collection.HTTP)).concat(_.values(order.HTTP)).concat(_.values(album.HTTP)).concat(_.values(save.HTTP))); //.do(x => console.log(x));

  (0, _analytics2['default'])(DOMactions, user, collection, upload, album, order);
...
},{"./album/album":1,"./album/editing":2,"./analytics":5,"./collection":6,"./helpers":9,"./order":12,"./photos":13,"./save":14,"./ui":15,"./upload":16,"./user":17,"./userinterface":18,"@cycle/core":19,"@cycle/dom":22,"@cycle/http":30,"immutable":93,"rx":101}],12:[function(require,module,exports){
'use strict';
...
var fields = ['firstName', 'lastName', 'email', 'address', 'zip', 'city', 'country'];

var formErrors = function formErrors(info) {
  return _.mapObject(info, function (val, key) {
    return !_.isEmpty(val);
  });
};

var extractFields = function extractFields(formID, fields) {
  var data = document.getElementById(formID).elements;
  return _.mapObject(_.pick(data, fields), function (val, key) {
    return val.value;
  });
};

var getQuantity = function getQuantity() {
  return ["qty1"].map(function (id) {
...
function intent(DOM, HTTP) {
  ...
  var actions = {
    ...
    formKeyUp$: DOM.select("#order-form input").events("input").debounce(100).map(function (ev) {
      return extractFields('order-form', fields);
    }),

    formSubmit$: btn('.submit-order-btn'),
    // changeQty$: DOM.select('#qty-selector').events('click').map(helpers.cancel).map(el => getQuantity()),

    changeQty$: btn('#qty-plus').map(1).merge(btn('#qty-minus').map(-1)),

    formSubmitted$: (0, _helpers.jsonPOST)(HTTP, /\/order/),

    clientToken$: HTTP.filter(function (res$) {
      return !_.isUndefined(res$.request.url);
    }).filter(function (res$) {
      return res$.request.url.match(/\/client_token/);
    }).mergeAll().map(function (res) {
      return res.text;
    }).share(),

    nonceReceived$: new _rx2['default'].Subject(),

    integrationReady$: new _rx2['default'].Subject()
  };

  actions.orderModal$.subscribe(function (ev) {
    return $('#order-modal').modal('show');
  });
  btn('.close-btn').subscribe(function (ev) {
    return $(ev.target['data-target']).modal('hide');
  });
...
var basePrice = 29.00;
var tenPagePrice = 5.00;
var additionalAlbumPrice = 25.00;
...
function model(actions, user, collection, album) {
  actions.clientToken$.subscribe(function (clientToken) {
    braintree.setup(clientToken, "dropin", {
      container: "payment-form",
      onPaymentMethodReceived: function onPaymentMethodReceived(obj) {
        return actions.nonceReceived$.onNext(obj);
      },
      onReady: function onReady(obj) {
        return actions.integrationReady$.onNext(obj);
      }
    });
  });

  var initial = (0, _immutable.fromJS)({ order: { 'qty': 1 }, info: {} });

  var updateInfos$ = actions.formKeyUp$.map(function (info) {
    return function (state) {
      return state.merge({ info: info });
    };
  });

  var checkForm$ = actions.formKeyUp$.map(formErrors).map(function (valid) {
    return function (state) {
      return state.merge({ valid: valid });
    };
  });

  var clientToken$ = actions.clientToken$.map(function (token) {
    return function (state) {
      return state.merge({ token: token });
    };
  });

  var hasIntegration$ = actions.integrationReady$.map(function (ready) {
    return function (state) {
      return state.merge({ ready: ready });
    };
  });

  var submitOrder$ = actions.formSubmit$.map(function (obj) {
    return function (state) {
      return state.set('status', -1);
    };
  });

  var submittedOrder$ = actions.formSubmitted$.map(function (obj) {
    return function (state) {
      return state.set('status', obj.status);
    };
  });

  var changedQty$ = actions.changeQty$.map(function (qty) {
    return function (state) {
      return state.updateIn(['order', 'qty'], function (q) {
        return Math.max(q + qty, 1);
      });
    };
  });

  var updateNonce$ = actions.nonceReceived$.withLatestFrom(user.state$, collection.state$, function (obj, user, collection) {
    return function (state) {
  ...
  return Observable.merge(clientToken$, updateInfos$, updateNonce$, hasIntegration$, submitOrder$, submittedOrder$, changedQty$, checkForm$).startWith(initial).scan(_helpers.apply).shareReplay(1);
}

function requests(actions, state$) {
  return {
    formSubmit$: actions.formSubmit$.withLatestFrom(state$, _helpers2['default'].argArray).filter(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 2);

      var ev = _ref2[0];
      var state = _ref2[1];
      return !_.isUndefined(state.get('valid')) && state.get('valid').values().every();
    }).map(function (_ref3) {
      var _ref32 = _slicedToArray(_ref3, 2);
    ...
    clientToken$: actions.orderModal$.withLatestFrom(state$, function (ev, state) {
      return state.get('token');
    }).filter(_.isUndefined).map({ url: '/client_token', 'method': 'GET', 'eager': true })
  };
}

var checkFields = function checkFields(fields) {
  return function (key) {
    return _.isEmpty(fields) ? '' : fields[key] ? '' : '.has-error';
  };
};

var countriesOptions = [(0, _cycleDom.h)('option', { 'value': '' }, "")].concat(_countries2['default'].map(function (c) {
  return (0, _cycleDom.h)('option', { 'value': c['Alpha2'] }, c['Name']);
}));

function view(state$) {
  return state$.map(function (state) {
    var check = checkFields(state.get('valid', (0, _immutable.Map)()).toJS());
    return (0, _cycleDom.h)('.modal.fade#order-modal', (0, _cycleDom.h)('.modal-dialog.modal-lg', (0, _cycleDom.h)('.modal-content', [(0, _cycleDom.h)('.modal-header', ''), (0, _cycleDom.h)('.modal-body', (0, _cycleDom.h)('.row', [(0, _cycleDom.h)('.col-lg-12', (0, _cycleDom.h)('.panel.panel-default', [
    // h('.panel-heading', h('h3.panel-title', "Album details")),
    (0, _cycleDom.h)('.panel-body.row', [(0, _cycleDom.h)('.col-lg-4', (0, _cycleDom.h)('img.img-responsive', { src: '/assets/images/a4.png' })), (0, _cycleDom.h)('.col-lg-5', [
    // h('p', i18('order.desc1')),
    (0, _cycleDom.h)('p', (0, _i18n2['default'])('order.desc2')), (0, _cycleDom.h)('p', (0, _i18n2['default'])('order.desc3'))]), (0, _cycleDom.h)('.col-lg-2.text-center', [(0, _cycleDom.h)('strong', 'Quantity'), (0, _cycleDom.h)('.quantity', [(0, _cycleDom.h)('h4', '' + state.get('order').get('qty')), (0, _cycleDom.h)('.btn-group', [(0, _cycleDom.h)('button.btn.btn-primary#qty-minus', (0, _cycleDom.h)('i.fa.fa-minus')), (0, _cycleDom.h)('button.btn.btn-primary#qty-plus', (0, _cycleDom.h)('i.fa.fa-plus'))])]), (0, _cycleDom.h)('h5', ["CHF " + qtyToPrice(state.get('order').get('qty')).toFixed(2)]), (0, _cycleDom.h)('small.text-muted', (0, _i18n2['default'])('order.shipping'))])])])), (0, _cycleDom.h)('.col-lg-8', (0, _cycleDom.h)('div', [
    // h('.panel-heading', h('h3.panel-title', "Delivery address")),
    (0, _cycleDom.h)('form.panel-body#order-form', [(0, _cycleDom.h)('.row', [(0, _cycleDom.h)('.col-lg-6.form-group' + check('firstName'), [(0, _cycleDom.h)('label', {
      'for': 'firstName'
    }, (0, _i18n2['default'])('order.firstname')), (0, _cycleDom.h)('input.form-control', {
      'name': 'firstName',
      'placeholder': ''
    })]), (0, _cycleDom.h)('.col-lg-6.form-group' + check('lastName'), [(0, _cycleDom.h)('label', {
      'for': 'lastName'
    }, (0, _i18n2['default'])('order.lastname')), (0, _cycleDom.h)('input.form-control', {
      'name': 'lastName',
      'placeholder': ''
    })])]), (0, _cycleDom.h)('.row', [(0, _cycleDom.h)('.col-lg-6.form-group' + check('email'), [(0, _cycleDom.h)('label', {
      'for': 'email'
    }, (0, _i18n2['default'])('order.email')), (0, _cycleDom.h)('.input-group', [(0, _cycleDom.h)('span.input-group-addon', '@'), (0, _cycleDom.h)('input.form-control', {
      'name': 'email',
      'type': 'email',
      'placeholder': (0, _i18n2['default'])('order.email-placeholder')
    })])]), (0, _cycleDom.h)('.col-lg-6.form-group' + check('address'), [(0, _cycleDom.h)('label', {
      'for': 'address'
    }, (0, _i18n2['default'])('order.address')), (0, _cycleDom.h)('input.form-control', {
      'name': 'address',
      'placeholder': (0, _i18n2['default'])('order.street')
    })])]), (0, _cycleDom.h)('.row', [(0, _cycleDom.h)('.col-lg-2.form-group' + check('zip'), [(0, _cycleDom.h)('label', {
      'for': 'zip'
    }, (0, _i18n2['default'])('order.zip')), (0, _cycleDom.h)('input.form-control', {
      'name': 'zip',
      'placeholder': ''
    })]), (0, _cycleDom.h)('.col-lg-5.form-group' + check('city'), [(0, _cycleDom.h)('label', {
      'for': 'city'
    }, (0, _i18n2['default'])('order.city')), (0, _cycleDom.h)('input.form-control', {
      'name': 'city',
      'placeholder': ''
    })]), (0, _cycleDom.h)('.col-lg-5.form-group' + check('country'), [(0, _cycleDom.h)('label', {
      'for': 'country'
    }, (0, _i18n2['default'])('order.country')), (0, _cycleDom.h)('select.form-control', {
      'name': 'country'
    }) //, countriesOptions)
    ])])])])), (0, _cycleDom.h)('.col-lg-4', (0, _cycleDom.h)('.panel.panel-default', [
    // h('.panel-heading', h('h4.panel-title', "Payment")),
    (0, _cycleDom.h)('.panel-body', (0, _cycleDom.h)('form', [(0, _cycleDom.h)('div#payment-form'), (0, _cycleDom.h)('button.btn.btn-info.pull-right#verify-btn', {
      'disabled': _.isUndefined(state.ready)
    }, (0, _i18n2['default'])('order.validate'))]))]))])), (0, _cycleDom.h)('.modal-footer', [_.isUndefined(state.status) ? (0, _cycleDom.h)('.clearfix', [_.isUndefined(state.get('order').get('nonce')) || _.isUndefined(state.get('order').get('qty')) ? '' : (0, _cycleDom.h)('button.btn.btn-primary.pull-right.submit-order-btn', { 'type': 'button', 'disabled': _.isUndefined(state.get('valid')) || !state.valid.values().every() }, ['Purchase ', (0, _cycleDom.h)('i.fa.fa-check')]), (0, _cycleDom.h)('button.btn.btn-default.pull-left.close-btn.order-cancel-btn', { 'data-target': '#order-modal' }, [(0, _i18n2['default'])('order.notnow') + ' ', (0, _cycleDom.h)('i.fa.fa-times')])]) : (0, _cycleDom.h)('.alert.alert-info.clearfix', state.status === -1 ? [(0, _cycleDom.h)('i.fa.fa-spin.fa-circle-o-notch.pull-right'), " " + (0, _i18n2['default'])('order.submitting')] : [(0, _cycleDom.h)('.fa.fa-2x.fa-smile-o.pull-right'), (0, _cycleDom.h)('p', (0, _i18n2['default'])('order.ordered')), (0, _cycleDom.h)('p', (0, _i18n2['default'])('order.confirmation')), (0, _cycleDom.h)('div', (0, _cycleDom.h)('button.btn.btn-primary.pull-right.close-btn', { 'data-target': '#order-modal' }, [(0, _i18n2['default'])('order.close') + " ", (0, _cycleDom.h)('i.fa.fa-times')]))])])])));
  });
...
module.exports = function (DOM, HTTP, user, collection) {
  ...
  var actions = intent(DOM, HTTP);
  var state$ = model(actions, user, collection);
  var requests$ = requests(actions, state$);
...
},{"./countries":8,"./helpers":9,"./i18n":10,"@cycle/dom":22,"immutable":93,"rx":101}],13:[function(require,module,exports){
'use strict';
...
module.exports = function (DOMactions, upload, collection) {
  var actions = {
    uploadedPhoto$: new _rx2['default'].Subject()
  };

  var demoPhotos$ = collection.actions.storedAlbum$.map(function (demo) {
    return function (y) {
      return demo.photos;
    };
  });
  var newPhotos$ = actions.uploadedPhoto$.map(function (file) {
    return function (photos) {
      return photos.concat(file);
    };
  });
  var clearPhotos$ = DOMactions.reset$.map(function (x) {
    return function (y) {
      return [];
    };
  });

  var state$ = Observable.merge(newPhotos$, clearPhotos$, demoPhotos$).startWith(_helpers.initial.photos).scan(_helpers.apply);
  // .do(x => console.log(x));
...
},{"./helpers":9,"rx":101}],14:[function(require,module,exports){
'use strict';
...
function view(state$) {
  return state$.map(function (state) {
    return [(0, _cycleDom.h)('.modal.fade#save-modal', (0, _cycleDom.h)('.modal-dialog.modal-md', (0, _cycleDom.h)('.modal-content', [(0, _cycleDom.h)('.modal-header', (0, _cycleDom.h)('button.close', { attributes: { 'data-dismiss': 'modal' }, innerHTML: '&times;' })), (0, _cycleDom.h)('.modal-body', [(0, _cycleDom.h)('p', 'Where do you want to receive the link to your album ?'), (0, _cycleDom.h)('form#save-form', [(0, _cycleDom.h)('.input-group', [(0, _cycleDom.h)('span.input-group-addon', '@'), (0, _cycleDom.h)('input.form-control', { type: 'email', name: 'email', placeholder: 'john@doe.com' }), (0, _cycleDom.h)('span.input-group-btn', (0, _cycleDom.h)('button.btn.btn-primary#submit-save-btn', state.status ? [(0, _cycleDom.h)('i.fa.fa-heart'), ' Saved'] : [(0, _cycleDom.h)('i.fa.fa-heart-o'), ' Save']))])])]), (0, _cycleDom.h)('.modal-footer')]))), state.status < 2 ? '' : (0, _cycleDom.h)('.alert.alert-info.fade.in#save-alert', [(0, _cycleDom.h)('i.fa.fa-check'), " Saved"])];
  });
}
...
module.exports = function (DOM, HTTP, user, collection, album) {
  ...
  var actions = {
    modal$: _helpers2['default'].btn(DOM, '#save-btn'),
    submit$: _helpers2['default'].btn(DOM, '#submit-save-btn').map(function (ev) {
      return document.getElementById("save-form").elements['email'].value;
    }),
    linkSent$: _helpers2['default'].jsonPOST(HTTP, /\/users\/\d+\/link\/\d+/),
    albumSaved$: _helpers2['default'].jsonPOSTResponse(HTTP, /\/save/)
  };

  var waitingForProcessing$ = _rx2['default'].Observable.merge(album.HTTP.createAlbum$.map(1), album.actions.createdAlbum$.map(-1)).startWith(0).scan(function (acc, v) {
    return acc + v;
  });

  actions.save$ = _rx2['default'].Observable.merge(album.state$.filter(function (a) {
    return a.size > 1;
  }), // && !_.some(a, _.isEmpty)),
  collection.state$.skip(2), actions.submit$).debounce(2000).withLatestFrom(waitingForProcessing$, function (save, waiting) {
    return waiting <= 0;
  }).filter(_.identity);

  actions.modal$.subscribe(function (ev) {
    $('#save-modal').modal('show');
    $('#save-form input[name=email]').focus();
  });

  actions.submit$.subscribe(function (email) {
    return user.actions.updateUser$.onNext({ email: email });
  });

  var state$ = _rx2['default'].Observable.merge(actions.linkSent$.map(function (res) {
    return function (state) {
      return _.extend(state, { 'status': 1 });
    };
  }), actions.albumSaved$.map(function (res) {
    return function (state) {
      return _.extend(state, { 'status': 2 });
    };
  }), actions.albumSaved$.delay(2000).map(function (res) {
    return function (state) {
      return _.extend(state, { 'status': 0 });
    };
  })).startWith({ 'status': 0 }).scan(_helpers2['default'].apply);

  var requests = {
    emailLink$: actions.submit$.withLatestFrom(user.state$, collection.state$, function (email, user, collection) {
      return {
    ...
    saveAlbum$: actions.save$.withLatestFrom(collection.state$, album.state$, function (ev, collection, album) {
      return {
        url: '/save',
        method: 'POST',
        eager: true,
        send: { collection: collection, album: album.toJS() }
      };
    }).filter(function (req) {
      return req.send.collection.id && req.send.collection.hash !== _helpers2['default'].demoID && req.send.album;
    })
  };
...
},{"./helpers":9,"@cycle/dom":22,"rx":101}],15:[function(require,module,exports){
'use strict';
...
exports.renderStartPage = renderStartPage;
...
function renderToolbar(collection, album, upload) {
  var loaded = album && album.length > 1 && collection.hash !== _helpers2['default'].demoID && !_.isUndefined(collection.id);
  return (0, _cycleDom.h)('div#nav.navbar.navbar-transparent.navbar-fixed-top', [(0, _cycleDom.h)('div.container-fluid', [(0, _cycleDom.h)('ul.nav.navbar-nav', (0, _cycleDom.h)('li', (0, _cycleDom.h)('a.navbar-brand', { href: '/' }, 'bigpiq'))), loaded ? (0, _cycleDom.h)('ul.nav.navbar-nav', [(0, _cycleDom.h)('li', (0, _cycleDom.h)('button.btn.btn-primary.navbar-btn#upload-btn', [(0, _cycleDom.h)('i.fa.fa-cloud-upload'), (0, _i18n2['default'])('nav.add')]))]) :
  // h('li',
  ...
  //     h('i.fa.fa-undo'), ' Redo'])),
  '', collection.hash === _helpers2['default'].demoID ? (0, _cycleDom.h)('.ul.nav.navbar-nav', [(0, _cycleDom.h)('li', (0, _cycleDom.h)('a.btn.navbar-btn.btn-info', { href: '/ui' }, [(0, _cycleDom.h)('i.fa.fa-flask'), ' ' + (0, _i18n2['default'])('nav.try')]))]) : '', (0, _cycleDom.h)('ul.nav.navbar-nav', renderProgressbar(upload)), loaded ? (0, _cycleDom.h)('ul.nav.navbar-nav.navbar-right', [
  // h('li.navbar-form',
  ...
  (0, _cycleDom.h)('li', (0, _cycleDom.h)('button.btn.btn-primary.navbar-btn#save-btn', [(0, _cycleDom.h)('i.fa.fa-heart-o'), (0, _i18n2['default'])('nav.save')])), (0, _cycleDom.h)('li', (0, _cycleDom.h)('button.btn.btn-primary.navbar-btn#order-btn', [(0, _cycleDom.h)('i.fa.fa-shopping-cart'), (0, _i18n2['default'])('nav.order')]))]) : '']), (0, _cycleDom.h)('input#file-input', { 'type': "file", 'name': "image", 'multiple': true })]);
}

function renderStartPage(collection) {
  return (0, _cycleDom.h)('.container.start-buttons', [(0, _cycleDom.h)('.row' + (!_.isUndefined(collection.id) ? '.step-disabled' : ''), [
  // h('.col-lg-12.text-center.step1', i18('front.step1')),
  (0, _cycleDom.h)('.col-lg-12.text-center', (0, _cycleDom.h)('button.btn.btn-info.btn-lg.btn-step#create-btn', {
    disabled: !_.isUndefined(collection.id)
  }, [(0, _cycleDom.h)('i.fa.fa-camera.fa-3x'), (0, _i18n2['default'])('front.upload')]))])]);
}
...
function renderProgressbar(upload) {
  if (upload.get('size', 0) > 0) {
    var progress = 100 * upload.get('files', []).size / upload.get('size');
    return progress < 100 ? [(0, _cycleDom.h)('li', (0, _cycleDom.h)('span.navbar-text', progress < 100 ? (0, _i18n2['default'])('ui.uploading') : (0, _i18n2['default'])('ui.processing'))), (0, _cycleDom.h)('li', (0, _cycleDom.h)('.progress.navbar-text', progress === 0 ? (0, _cycleDom.h)('.progress-bar.progress-bar-info.progress-bar-striped.active', { style: { width: '100%' } }) : (0, _cycleDom.h)('.progress-bar.progress-bar-info', {
...
},{"./helpers":9,"./i18n":10,"@cycle/dom":22,"rx":101}],16:[function(require,module,exports){
'use strict';
...
function intent(DOM, DOMactions, collection) {
  _helpers2['default'].btn(DOM, '#upload-area').subscribe(function (_) {
    return document.getElementById('file-input').dispatchEvent(new MouseEvent('click'));
  });

  DOM.select('#upload-area').events('dragover').map(_helpers2['default'].cancel).subscribe(function (ev) {
    ev.dataTransfer.dropEffect = 'copy';return ev;
  });

  var toggleUpload$ = _helpers2['default'].btn(DOM, '#upload-btn').merge(_helpers2['default'].btn(DOM, '#create-btn')).merge(_helpers2['default'].btn(DOM, '#addmore-btn'));

  var startUpload$ = DOMactions.selectFiles$.flatMapLatest(function (files) {
    return collection.state$.filter(_helpers2['default'].hasID).take(1).map(function (c) {
      return [files, c];
    });
  }).share();

  startUpload$.subscribe(function (ev) {
    $('#album-title').focus();
    // if ($('#step2').offset()) {
    // $('html,body').animate({
  ...
  var fileUpload$ = startUpload$.flatMapLatest(function (_ref) {
    var _ref2 = _slicedToArray(_ref, 2);

    var files = _ref2[0];
    var collection = _ref2[1];
    return _rx2['default'].Observable.from(files).scan(function (acc, el) {
      return acc.concat(el);
    }, []);
  }).publish();

  var uploadedFiles$ = fileUpload$.withLatestFrom(startUpload$, function (uploaded, _ref3) {
    var _ref32 = _slicedToArray(_ref3, 2);

    var files = _ref32[0];
    var collection = _ref32[1];
    return { uploaded: uploaded, files: files };
  }).filter(function (_ref4) {
    var uploaded = _ref4.uploaded;
  ...
  var uploaded$ = new _rx2['default'].Subject();
  var finished$ = new _rx2['default'].Subject();

  return { toggleUpload$: toggleUpload$, startUpload$: startUpload$, fileUpload$: fileUpload$, uploadedFiles$: uploadedFiles$, uploaded$: uploaded$, finished$: finished$ };
}

function events(DOMactions, actions) {
  actions.toggleUpload$.subscribe(function (ev) {
    return $('#upload-modal').modal('show');
  });
  DOMactions.selectFiles$.subscribe(function (x) {
    return $('#upload-modal').modal('hide');
  });
}

var initial = _immutable2['default'].fromJS({ 'files': [], 'size': 0 });

function model(actions, DOMactions) {

  var startedFunc$ = actions.startUpload$.map(function (_ref6) {
    var _ref62 = _slicedToArray(_ref6, 2);

    var files = _ref62[0];
    var collection = _ref62[1];
    return function (upload) {
      return upload.set('size', files.length);
    };
  });

  var uploadedFunc$ = actions.uploaded$.map(function (photo) {
    return function (upload) {
      return upload.update('files', _immutable2['default'].List(), function (f) {
        return f.push(photo);
      });
    };
  });
  ...
  return _rx2['default'].Observable.merge(startedFunc$,
  // uploadedFunc$,
  uploadedFunc$
  // finishedFunc$
  ).startWith(initial).scan(_helpers2['default'].apply).shareReplay(1);
}

function view(state$) {
  return state$.map(function (state) {
    return (0, _cycleDom.h)('.modal.fade#upload-modal', (0, _cycleDom.h)('.modal-dialog', (0, _cycleDom.h)('.modal-content#upload-area', (0, _cycleDom.h)('.modal-body', [(0, _cycleDom.h)('.message', (0, _i18n2['default'])('upload.dragclick')), (0, _cycleDom.h)('.fa.fa-download.fa-3x')]))));
  });
}

module.exports = function (DOM, DOMactions, collection) {
  var actions = intent(DOM, DOMactions, collection);
...
},{"./helpers":9,"./i18n":10,"@cycle/dom":22,"immutable":93,"rx":101}],17:[function(require,module,exports){
'use strict';
...
var COOKIE = 'bigpiquserid';

function intent(HTTP) {
  return {
    gotCookie$: Observable['return'](_cookies2['default'].getItem(COOKIE)).map(function (id) {
      return id && id.match(/\d+/) ? id : null;
    }),
    gotUser$: (0, _helpers.jsonGETResponse)(HTTP, /^\/users\/\d+$/),
    createdUser$: (0, _helpers.jsonPOST)(HTTP, /^\/users$/),
    updateUser$: new _rx2['default'].Subject()
  };
}

function model(actions) {
  var newUser$ = Observable.merge(actions.gotUser$.filter(function (res) {
    return res.status !== 404;
  }).map(function (res) {
    return res.body;
  }), actions.createdUser$);

  var updateUser$ = actions.updateUser$.map(function (obj) {
    return function (user) {
      return _.extend(user, obj);
    };
  });

  newUser$.filter(_helpers.hasID).subscribe(function (user) {
    console.log(user.id);
    _cookies2['default'].setItem(COOKIE, user.id);
  });

  return _rx2['default'].Observable.merge(newUser$.map(function (newuser) {
    return function (user) {
      return newuser;
    };
  }), updateUser$).startWith({}).scan(_helpers.apply).shareReplay(1);
}

function requests(DOMactions, actions, user$) {
  return {
    getUser$: actions.gotCookie$.filter(_.identity).map(function (id) {
      return '/users/' + id;
    }),

    createUser$: Observable.merge(user$.skip(1).filter(_helpers.hasNoID), actions.gotUser$.filter(function (res) {
      return res.status === 404;
    }), actions.gotCookie$.filter(function (id) {
      return id === null;
    })).take(1).zip(DOMactions.selectFiles$.take(1)).map(function (x) {
      return {
        method: 'POST',
        url: '/users',
        send: {}
    ...
    updateUser$: actions.updateUser$.flatMapLatest(function (ev) {
      return user$.take(1).map(function (user) {
        return {
          url: '/users',
          method: 'POST',
          send: user
...
module.exports = function (HTTP, DOMactions) {
  var actions = intent(HTTP);
  var state$ = model(actions);
  var requests$ = requests(DOMactions, actions, state$);
...
},{"./cookies":7,"./helpers":9,"rx":101}],18:[function(require,module,exports){
'use strict';
...
module.exports = function (DOMactions, album) {
  ...
  function model(DOMactions, album) {
    var uploading$ = DOMactions.selectFiles$.map(function (f) {
      return function (ui) {
        return (ui | _helpers.UI.uploading) & ~_helpers.UI.processing;
      };
    });
    var processing$ = album.HTTP.createAlbum$.map(function (f) {
      return function (ui) {
        return (ui ^ _helpers.UI.processing) & ~_helpers.UI.uploading;
      };
    });
    var complete$ = album.actions.createdAlbum$.map(function (f) {
      return function (ui) {
        return ui & ~(_helpers.UI.uploading | _helpers.UI.processing | _helpers.UI.uploadBox);
      };
    });

    var state$ = Observable.merge(uploading$, processing$, complete$).startWith(_helpers.initial.ui).scan(function (ui, func) {
      return _.extend(ui, { state: func(ui.state) });
    }).shareReplay(1);

    return state$;
  }

  var state$ = model(DOMactions, album);

  return { state$: state$ };
};

},{"./helpers":9,"rx":101}],19:[function(require,module,exports){
"use strict";
...
},{"rx":101}],20:[function(require,module,exports){
"use strict";
...
},{"./render-dom":25,"rx":101}],21:[function(require,module,exports){
"use strict";
...
},{"./custom-element-widget":20,"es6-map":80}],22:[function(require,module,exports){
"use strict";
...
},{"./mock-dom-response":24,"./render-dom":25,"./render-html":26,"./virtual-hyperscript":28,"virtual-dom/virtual-hyperscript/svg":128}],23:[function(require,module,exports){
"use strict";
...
},{"rx":101}],24:[function(require,module,exports){
'use strict';
...
},{"rx":101}],25:[function(require,module,exports){
"use strict";
...
},{"./custom-elements":21,"./fromevent":23,"./transposition":27,"./virtual-hyperscript":28,"matches-selector":98,"rx":101,"vdom-parser":107,"virtual-dom/diff":114,"virtual-dom/patch":115}],26:[function(require,module,exports){
"use strict";
...
},{"./custom-element-widget":20,"./custom-elements":21,"./transposition":27,"rx":101,"vdom-to-html":111}],27:[function(require,module,exports){
"use strict";
...
},{"rx":101,"virtual-dom/vnode/vnode":136}],28:[function(require,module,exports){
/* eslint-disable */
'use strict';
...
/* eslint-enable */
},{"virtual-dom/virtual-hyperscript/hooks/ev-hook.js":123,"virtual-dom/virtual-hyperscript/hooks/soft-set-hook.js":124,"virtual-dom/virtual-hyperscript/parse-tag.js":126,"virtual-dom/vnode/is-thunk":130,"virtual-dom/vnode/is-vhook":131,"virtual-dom/vnode/is-vnode":132,"virtual-dom/vnode/is-vtext":133,"virtual-dom/vnode/is-widget":134,"virtual-dom/vnode/vnode.js":136,"virtual-dom/vnode/vtext.js":138,"x-is-array":141}],29:[function(require,module,exports){
"use strict";
...
},{"rx":101,"superagent":106}],30:[function(require,module,exports){
"use strict";
...
},{"./http-driver":29}],31:[function(require,module,exports){
...
},{"./valid-callable":62,"./valid-value":63}],45:[function(require,module,exports){
'use strict';
...
},{"es5-ext/object/primitive-set":58}],84:[function(require,module,exports){
'use strict';
...
},{"./iterator-kinds":83,"d":36,"es5-ext/object/set-prototype-of":59,"es6-iterator":71,"es6-symbol":86}],85:[function(require,module,exports){
'use strict';
...
},{"./is-native-implemented":82,"./lib/iterator":84,"d":36,"es5-ext/array/#/clear":37,"es5-ext/array/#/e-index-of":38,"es5-ext/object/set-prototype-of":59,"es5-ext/object/valid-callable":62,"es5-ext/object/valid-value":63,"es6-iterator/for-of":69,"es6-iterator/valid-iterable":79,"es6-symbol":86,"event-emitter":91}],86:[function(require,module,exports){
arguments[4][73][0].apply(exports,arguments)
},{"./is-implemented":87,"./polyfill":88,"dup":73}],87:[function(require,module,exports){
'use strict';
...
},{"d":36}],89:[function(require,module,exports){
/*!
 * escape-html
...
},{"individual/one-version":95}],91:[function(require,module,exports){
'use strict';
...
},{"d":36,"es5-ext/object/valid-callable":62}],92:[function(require,module,exports){
(function (global){
...
},{"min-document":31}],93:[function(require,module,exports){
/**
...
},{"./index.js":94}],96:[function(require,module,exports){
"use strict";
...
},{"sentence-case":102}],100:[function(require,module,exports){
...
},{"_process":33}],102:[function(require,module,exports){
...
},{"./vendor/camel-case-regexp":103,"./vendor/non-word-regexp":104,"./vendor/trailing-digit-regexp":105,"lower-case":97}],103:[function(require,module,exports){
...
},{"emitter":34,"reduce":100}],107:[function(require,module,exports){
...
},{"./namespace-map":108,"./property-map":109,"virtual-dom/vnode/vnode":136,"virtual-dom/vnode/vtext":138}],108:[function(require,module,exports){
...
},{"./property-config":112,"escape-html":89}],111:[function(require,module,exports){
var escape = require('escape-html');
...
},{"./create-attribute":110,"./void-elements":113,"escape-html":89,"param-case":99,"virtual-dom/virtual-hyperscript/hooks/attribute-hook":122,"virtual-dom/virtual-hyperscript/hooks/soft-set-hook":124,"virtual-dom/vnode/is-thunk":130,"virtual-dom/vnode/is-vnode":132,"virtual-dom/vnode/is-vtext":133,"virtual-dom/vnode/is-widget":134,"xtend":142}],112:[function(require,module,exports){
/**
 * Attribute types.
 */
...
},{"./vtree/diff.js":140}],115:[function(require,module,exports){
var patch = require("./vdom/patch.js")

module.exports = patch

},{"./vdom/patch.js":120}],116:[function(require,module,exports){
var isObject = require("is-object")
...
},{"../vnode/is-vhook.js":131,"is-object":96}],117:[function(require,module,exports){
...
},{"../vnode/handle-thunk.js":129,"../vnode/is-vnode.js":132,"../vnode/is-vtext.js":133,"../vnode/is-widget.js":134,"./apply-properties":116,"global/document":92}],118:[function(require,module,exports){
...
},{"../vnode/is-widget.js":134,"../vnode/vpatch.js":137,"./apply-properties":116,"./update-widget":121}],120:[function(require,module,exports){
var document = require("global/document")
var isArray = require("x-is-array")
...
},{"./create-element":117,"./dom-index":118,"./patch-op":119,"global/document":92,"x-is-array":141}],121:[function(require,module,exports){
var isWidget = require("../vnode/is-widget.js")
...
},{"../vnode/is-widget.js":134}],122:[function(require,module,exports){
'use strict';
...
},{"ev-store":90}],124:[function(require,module,exports){
'use strict';
...
},{"../vnode/is-thunk":130,"../vnode/is-vhook":131,"../vnode/is-vnode":132,"../vnode/is-vtext":133,"../vnode/is-widget":134,"../vnode/vnode.js":136,"../vnode/vtext.js":138,"./hooks/ev-hook.js":123,"./hooks/soft-set-hook.js":124,"./parse-tag.js":126,"x-is-array":141}],126:[function(require,module,exports){
'use strict';
...
},{"browser-split":32}],127:[function(require,module,exports){
'use strict';
...
},{"./hooks/attribute-hook":122,"./index.js":125,"./svg-attribute-namespace":127,"x-is-array":141}],129:[function(require,module,exports){
...
},{"../vnode/is-vhook":131,"is-object":96}],140:[function(require,module,exports){
var isArray = require("x-is-array")
...
},{"../vnode/handle-thunk":129,"../vnode/is-thunk":130,"../vnode/is-vnode":132,"../vnode/is-vtext":133,"../vnode/is-widget":134,"../vnode/vpatch":137,"./diff-props":139,"x-is-array":141}],141:[function(require,module,exports){

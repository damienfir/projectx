...
case class UpdatePages(pages: Pot[List[Page]])
object UpdatePages { def apply(page: Page) = new UpdatePages(Ready(List(page))) }

case class UpdateFromHash(album: Album)
case class GetAlbum(hash: String)
case class SetAlbum(album: Pot[Album])
